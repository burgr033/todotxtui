...
class SimpleTodoList:
    ...
    def read_todo_file(self):
        ...
        if os.path.exists(config["todo_file_path"]):
            todo_fp = open(config["todo_file_path"], "r", encoding="utf-8")
    ...
    def replace_keywords(self, item_to_work):
        item_to_work = item_to_work.replace("due:today", "due:{0}".format(today))
        item_to_work = item_to_work.replace("due:tomorrow", "due:{0}".format(tomorrow))
    ...
    def launch_decrypt(self, password):
        crypto.decrypt(bytes(password, "utf-8"), todo_file)
        self.read_todo_file()

    def ask_password(self):
        self.master.show_text_box_popup("Password", self.verify_password, password=True)

    def verify_password(self, password):
        global glob_password
        glob_password = password
        returnValue = crypto.check_hashes(password, pw_hash)
        if returnValue:
            self.launch_decrypt(password)
        else:
            self.ask_password()
    ...
    def color_date(self):
        ...
        self.todo_scroll_cell.add_text_color_rule(
            "due:{0}".format(tomorrow),
            py_cui.WHITE_ON_YELLOW,
            "contains",
            match_type="regex",
        )
    ...
    def color_context(self):
        self.todo_scroll_cell.add_text_color_rule(
            "(@[^\s]+)", py_cui.GREEN_ON_BLACK, "contains", match_type="regex"
        )
        self.in_progress_scroll_cell.add_text_color_rule(
            "(@[^\s]+)", py_cui.GREEN_ON_BLACK, "contains", match_type="regex"
        )
    ...
    def color_project(self):
        ...
        for project in filtered_projects:
            ...
            self.todo_scroll_cell.add_text_color_rule(
                project.replace("+", "\+"), color_choice, "contains", match_type="regex"
            )
            self.in_progress_scroll_cell.add_text_color_rule(
                project.replace("+", "\+"), color_choice, "contains", match_type="regex"
            )
    ...
    def __init__(self, master):
        ...
        self.todo_scroll_cell.set_selected_color(py_cui.BLACK_ON_WHITE)

        # color of selected Item is BLACK_ON_WHITE
        self.in_progress_scroll_cell.set_selected_color(py_cui.BLACK_ON_WHITE)

        # color of selected Item is BLACK_ON_WHITE
        self.done_scroll_cell.set_selected_color(py_cui.BLACK_ON_WHITE)

        # press s in overview mode to safety save the file.
        self.master.add_key_command(py_cui.keys.KEY_S_LOWER, self.save_todo_file)
        ...
        self.master.add_key_command(py_cui.keys.KEY_R_LOWER, self.read_todo_file)

        # press f to open the search form and reset any marked lines from prior search
        self.master.add_key_command(py_cui.keys.KEY_F_LOWER, self.open_find_form)
        ...
        self.master.add_key_command(py_cui.keys.KEY_O_LOWER, self.open_todotxt_file)

        # for quick cycling widgets press TAB
        self.master.set_widget_cycle_key(forward_cycle_key=py_cui.keys.KEY_TAB)
...
def encrypt():
    crypto.encrypt(bytes(glob_password, "utf-8"), todo_file)


# set some titles...
root.run_on_exit(command=encrypt)
